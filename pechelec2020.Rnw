...
électrique sur le bassin de la Vilaine de 1998 à 2020.},
...
electrofishing in the Vilaine watershed from 1998 to 2020.
}
...
\begin{abstract}
 ...
 affluents de petite taille à des distances croissantes de l'estuaire.     
    ...
    fluvial de civelles sur le bassin versant a très fortement augmenté.
...
\begin{itshape}
  ...
  migratory pathway for eel in the Vilaine watershed. They have then diminished rapidly from 2003
  untill a low level in 2011, and
  ...
  fluvial recruitment on the Vilaine basin has been largely increased. We do observe 
  an increase in young age classes in the downstream par of the basin (area <50
...
load(str_c("C:/workspace/pdata/traitement_stacomi/rapport_anguille/data/",CY,"/vvv.Rdata"))
...
#serie<-read_excel(path=str_c(datawd,"serie_pro_vilaine.xlsx"),
...
serie$passe_agj_cohorte <- round(rowSums(cohort[as.character(1996:(CY)), 2:6], na.rm = TRUE))
...
recagj <- recagj[recagj>10] # remove last operation year
...
barrage d'Arzal. La même figure est reportée en kg pour les civelles (Figure
\ref{figure2recrutement_kg}).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/figure2recrutement.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/figure2recrutement_kg.pdf}
...
#nrow(pec)#370
# quelles sont les stations "gardées"
...
#densityplot(log(pec$densCS))
car::qqPlot(log(pec$densCS+0.01))

shapiro.test(dens$densCS+0.01) #W = 0.74, p-value < 2.2e-16   #   Performs the Shapiro-Wilk test of normality.  p<0.1 données non normales

shapiro.test(log(dens$densCS+0.01)) # W = 0.98, p-value = 0.01 NO LONGER OK !!!
...
shapiro.test(resid(glmdens)) # 0.6 OK ouf !!!
...
#DEPRECATED see figure denste annee (which is not the right model ) and figure7
...
load(file=str_c(datawdy,"dens.cld.Rdata"))
...
pdf(file=str_c(imgwdy,"dens_annee.pdf"),height=5,width=9)
...
g+geom_point(col="grey40",size=1.8)+
    ...
    geom_line(aes(x=annee,y=D,group=1),data=tableau2,col="red",shape=3)+
	#colorspace::scale_fill_discrete_sequential(name="group",palette = "ag_Sunset", nmax = 12, order = 2:12)+
...
pdf(file=str_c(imgwdy,"biom_annee.pdf"),height=5,width=9)
...
dev.off()
@
<<figure_biom_annee_dist,echo=FALSE,eval=TRUE,results=hide>>=
########################################
#  Figure  Tendance des biomasses en fonction des années et de la distance mer
########################################
cols<-rep(RColorBrewer::brewer.pal(12,"Set3"),4)[1:length(unique(dens$annee))]
pdf(file=str_c(imgwdy,"biom_annee_dist.pdf"),height=5,width=9)
g<-ggplot(dens,aes(x=annee,y=biomCS))
g+geom_boxplot(aes(fill=annee))+
	facet_grid(~ classdist)+
	scale_fill_manual(name="Année",values=cols)+
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\small
\input{../../pdata/pechelec/table/2020/tableau2_dens_eff_annee.tex}
\normalsize
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Entre 2015 et 2020 elle varie entre \Sexpr{tableau2["2017",3]} en
2017 et \Sexpr{tableau2["2018",3]} en 2018.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{figure}[htbp]
\centering
 \includegraphics[width=0.5\textwidth]{2020/dens_annee.pdf} 
...
\Sexpr{tableau2["2001",7]} (+-\Sexpr{tableau2["2001",8]}) g.m$^{-2}$) puis en chute de 2005 jusqu'en 2017
à \Sexpr{tableau2["2017",7]}  (+-\Sexpr{tableau2["2017",8]}) g.m$^{-2}$ (Tableau \ref{tableau2_dens_eff_annee}
et Figure \ref{biom_annee}) avant une réaugmentation de 2018 à 2020 à
\Sexpr{tableau2["2020",7]}  (+-\Sexpr{tableau2["2020",8]}).


\begin{figure}[htbp]
\centering
 \includegraphics[width=0.5\textwidth]{2020/biom_annee.pdf} 
...
\begin{figure}[htbp]
  \centering
    \includegraphics[width=0.5\textwidth]{2020/dens_annee_dist.pdf}
...
\small
\input{../../pdata/pechelec/table/2020/tableau3_dens_dist_annee.tex}
\normalsize

\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\textwidth]{2020/biom_annee_dist.pdf}
...
\small
\input{../../pdata/pechelec/table/2020/table_biom_dist_annee.tex}
\normalsize
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/dens_age_annee.pdf}
...
marq <- marq[!is.na(marq$annee),]
nrow(marq) # 686
marq$station[!is.element(marq$station,summary_station$station)]
...
sum(marq$marque%in%marqr) #92 (2015) 126 (2016) 206(2017) 244(2018) 282(2020)
...
xta1<-xtable(ta1,
	...
	caption = c(
		...
		str_c("Marquage et recaptures. Les marques sont soit posées, soit lues lorsque l'anguille est déjà marquée.")))
digits(xta1)<-c(0,0,0)
align(xta1) <- c("l","r","r")
print(xta1,
...
# croissance[croissance$croissance_annuelle_mm<0,]
croissance$croissance_annuelle_mm[croissance$croissance_annuelle_mm<0]<-0
croissance$croissance_taille[croissance$croissance_taille<0] <- 0
...
ggplot(croissance)+geom_boxplot(aes(x=annees,y=croissance_annuelle_mm,fill=annees, group=annees))+
	ylab("croissance en mm")+xlab("duree du marquage")+
	viridis::scale_fill_viridis("Année",option="plasma",discrete=TRUE)+	
	theme_bw()
dev.off()
...
# en passant par un gam pas mieux l'effet de diminution de la croissance lors des deux premières années n'est pas significatif.
gbest <- glm(croissance_annuelle_mm~station, data=croissance)
anova(g0,g1,g2, test="Chisq")
Anova(g0,type="III",test="F")
Anova(g1,type="III",test="F")
Anova(g2,type="III",test="F")
Anova(gbest,type="III",test="F")
options("xtable.include.rownames"=TRUE)
print(xtable(Anova(gbest,type="III",test="F"),
        caption="Test statistique (Anova) des variables du modèle croissance annuelle $\\sim$ station,
            où $station$ correspond la station de pêche. Deux autres facteurs, année et taille au moment du marquage,
...
@
Les recaptures et marquages effectués de 2009 à 2020 sont indiqués au Tableau
\ref{tablemr}.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{../../pdata/pechelec/table/2020/tableaumr1.tex}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\Sexpr{nrow(croissance)}), les croissances annuelles s'établissent à
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/croissance.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/croissance2.pdf}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/croissance3.pdf}
\caption[Croissance annuelles des anguilles marquées]{Croissance annuelle des
...
\input{../../pdata/pechelec/table/2020/anova_croissance.tex}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{2020/croissance4.pdf}
...
dessus de cette cible à partir de 2015 (Figure
...
certaines stations aval, où les recaptures ont été très faibles (ruisseau de la Bouloterie, Canut Nord et Arzal, ruisseau des
Arches proches de la rupture d'écoulement).

Globalement, l'examen des densités après 2011 montre que ces dernières ont
ré-augmenté a des niveaux proches de ceux de 1998 à 2012 sur la partie aval.
Elles restent plus faibles sur le secteur intermédiaire alors qu'on aurait pu
s'attendre à un effet marqué des transports.
...
quota a produit 4 années sur 11 entre 2009 et 2020 un
...
densités, dans d'autres cas ils ne sont pas détectables immédiatement, dans tous
les cas les tailles d'anguilles observées restent faibles, il est possible que
la croissance soit beaucoup plus faible que celle utilisée dans la
...
1990. Il est difficile d'attribuer cette augmentation au recrutement par la
passe ou par les transports après 2019.
\item Oust et Aff 2014 $\Longleftrightarrow$ Arz (augmentation 2014 - forte
variabilité) et Arches (très faible augmentation après 2018).
\item  Vilaine entre Guipry et Molière 2015 $\Longleftrightarrow$ Canut Nord,
forte augmentation en 2018, Aron augmentation à partir de 2018.
\item  Vilaine entre Bourg des Comptes et Champcors 2016 $\Longleftrightarrow$
Canut Nord forte augmentation en 2018, Seiche (nouvelles stations)- densités
importantes observées sur radier en 2019.
\item  Vilaine confluence Oust à Malon 2017 $\Longleftrightarrow$
Canut Sud (augmentation en 2019) et Aron (augmentation en
2016...).
\end{itemize}\\
...
Il est à noter que globalement, les densités sont cohérentes dans leur tendance 
d'une année sur l'autre et qu'en conséquence les tendances
...
civelles ayant colonisé le bassin, grace aux opérations de transport ou au
...
\section{Annexes}
\small
\input{../../pdata/pechelec/table/2020/star.tex}
\normalsize
%\begin{figure}[htbp]
%\centering
%\includegraphics[width=0.8\textwidth]{2020/densiteannuelle.pdf}
%\caption[Tendance des densités modèle]{Classement des densités en fonction des
%années, test Post hoc de Tukey sur le modèle $log(dens) \sim a +s +
%\epsilon(ss)$, avec $\epsilon(s) \sim N(0,\sigma_s^2)$ a=année, s=stations,
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{2020/dens_age_annee_dist.pdf}
...
taille âge) et la distance à la mer pour les pêches électriques de 1998 à 2020.
La courbe en rouge indique le seuil du plan de gestion.}
\label{dens_age_annee_dist}
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{../../pdata/pechelec/table/2020/table_densite_age_annee.tex}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{2020/residus.pdf}
...
\begin{figure}[htbp]
\centering
 \includegraphics[width=0.8\textwidth]{2020/dens_annee_riviere}
...
% /pdata/traitement_stacomi/rapport_anguille/image/2020/recrutement_vilaine_europeen.png
\begin{figure}[htpb]
\centering
\includegraphics[width=0.8\textwidth]{2020/recrutement_vilaine_europeen.png}

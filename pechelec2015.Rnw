...
imgwd<-"F:/workspace/pdata/pechelec/image/"
tabwd<-"F:/workspace/pdata/pechelec/table/"
...
<<tableau1,echo=FALSE,eval=TRUE,results=hide>>=
...
ges<-read.table(str_c(datawd,"export_gestion.csv"),sep=";",header=TRUE)
...
\Sexpr{sn(recciv[quelcmin],3)} en \Sexpr{ges$year[quelcmin]} à
\Sexpr{sn(recciv[quelcmax],3)} millions de civelle par an en 
\Sexpr{ges$year[quelcmax]}. 
Le nombre d'anguilles jaunes comptées sur les passes a varié de
\Sexpr{sn(ges$yellow[quelymin],0)} à  \Sexpr{sn(ges$yellow[quelymax],0)} entre
la plus mauvaise année \Sexpr{ges$year[quelymin]} et la meilleure
\Sexpr{ges$year[quelymax]} (Figure \ref{figure2recrutement}).  Entre 2000 et 2005, les captures
...
stations=read.table(str_c(datawd,"code_stations.csv"),sep=";",header=TRUE)
dens0<-merge(pec,stations,by="Id_PtPrelMetier")
stations=read.table(str_c(datawd,"code_stations2014.csv"),sep=";",header=TRUE)
dens1<-merge(pec,stations,by="Id_PtPrelMetier")
dens<-rbind(dens0,dens1)
#head(dens)
...
shapiro.test(log(dens$densCS)) # W = 0.99067, p-value = 0.1155
...
cat(star,file=str_c(tabwdy,"star.tex"))
############################################################
...
@
<<figure7_anx_glht,echo=FALSE,eval=TRUE,results=hide>>=
...
plot(dens.cld,
		col=c("green", "#98ED5E", "green","#98ED5E","green","turquoise","slateblue","darkblue","blue","darkblue","blue","slateblue","slateblue"),
		ylab="log (densite)")
par(old.par)
dev.off()
@
<<figure_densite_annee,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
dev.off()
@
<<figure_densite_annee_dist,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
print(xtableau3,file=str_c(tabwdy,"tableau3_dens_dist_annee.tex"),
		...
		sanitize.colnames.function = identity)
@
<<figurebiom_annee,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
print(xtableau4,file=str_c(tabwdy,"table_biom_dist_annee.tex"),
		...
		sanitize.colnames.function = identity)
@
<<figure_densite_age_annee,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
print(xtableau5,file=str_c(tabwdy,"table_densite_age_annee.tex"),
		...
		sanitize.colnames.function = identity)
@
<<figure_age_annee_dist,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
dev.off()
@
<<figure_anx_densite_annee_riviere,echo=FALSE,eval=TRUE,results=hide>>=
########################################
...
\Sexpr{max(tableau2[,5])} (Tableau \ref{tableau2_dens_eff_annee}). La densité moyenne par station est
passée de \Sexpr{tableau2["1998",3]} (+-\Sexpr{tableau2["1998",4]}) anguille.m$^{-2}$ en 1998 
à \Sexpr{tableau2["1999",3]} (+-\Sexpr{tableau2["1999",4]}) anguille.m$^{-2}$ en 2000 
...
légèrement à \Sexpr{tableau2["2013",3]}
(+-\Sexpr{tableau2["2013",4]}) anguille.m$^{-2}$ en 2013
...
\Sexpr{tableau3["2013",3]} anguille.m$^{-2}$).\\
...
\Sexpr{tableau2["2001",7]} (+-\Sexpr{tableau2["2001",8]}) g.m$^{-2}$) puis en chute marquée de 2005 jusqu'en 2011 à \Sexpr{tableau2["2011",7]}  (+-\Sexpr{tableau2["2011",8]}) g.m$^{-2}$ et une légère augmentation en 2013  \Sexpr{tableau2["2013",7]} (+-\Sexpr{tableau2["2013",8]}) g.m$^{-2}$ (Tableau \ref{tableau2_dens_eff_annee}
...
du même ordre pour les âges 3 et 4+ \Sexpr{tableau5["2013",5]}, mais les
...
ggplot(croissance)+geom_point(aes(x=taille0,y=croissance_annuelle_mm,shape=duree_an_round),col="black",size=3.5)+
		...
		scale_shape_manual("Nb années",values=c(16,17,18,19,20,21))+
		theme_bw()
dev.off()
tapply(croissance$croissance_annuelle_mm,croissance$duree_an_round,mean)
pdf(file=str_c(imgwdy,"croissance3.pdf"),height=6,width=6)
ggplot(croissance)+geom_boxplot(aes(x=duree_an_round,y=croissance_annuelle_mm,col=duree_an_round))+
...
taux de recapture des anguilles est de \Sexpr{txrecapt} \%. 
...
fort pourcentage de recapture sur les stations (\Sexpr{txrecapt} \%) est à
noter. Il indique que les anguilles de plus de 30 cm sont globalement
sédentaires.\\ 

L'augmentation des densités sur le bassin versant concernait également, en 2013,
...
\ref{dens_annee_riviere} en annexe). Cette tendance était beaucoup plus
difficile à expliquer que l'augmentation sur les secteurs aval.
Elle correspondait à des anguilles entre 20 et 40 cm. Si l'on se réfère aux taux de croissance très
...
d'arrivée donnée.  La passe piège de Malon (premier ouvrage sur la Vilaine) a été
démontée en septembre 2008 et c'est seulement en mai 2011 qu'elle a été
reconstruite.
Il est possible qu'il y ait eu un blocage au niveau de ce barrage.Cette tendance n'a pas été retrouvée les années suivantes où
les densités ont continué à diminuer, il s'agissait probablement d'anguilles en
migration. \\ \mediumskip

En conclusion, les suivis de 2014 et 2015 confirment que la
croissance des anguilles est faible sur les affluents de la Vilaine. Une
opération  d'ageage des anguilles est en cours pour confirmer ces résultats.
...
\Sexpr{sn(ges[15,"recr_flu_aj_m1"])} en 2010 à
\Sexpr{sn(ges[18,"recr_flu_aj_m1"])} en 2013.
...
\Sexpr{print(version$version.string)} sur une plateforme

...
%% page settings later!
\documentclass[10pt,twocolumn,titlepage,twoside]{article}
\usepackage{float}
...
\begin{minipage}{\textwidth}
\begin{center}
 \rotatebox{270}{\includegraphics[width=0.5\paperwidth]{../peche.jpg}}
\end{center}
\end{minipage}
...
<< knitropt, eval=TRUE, include=FALSE >>=
...
{
  if (!is.character(x))
    stop("Package should be a string")
...
load_package("knitr")
opts_knit$set(base.dir = "C:/workspace/passe_anguilles/image")
opts_knit$set(
  eval = FALSE,
  message = FALSE,
  warnings = FALSE,
...
imgwd <- "C:/workspace/pechelec/image/"
tabwd <- "C:/workspace/pechelec/table/"
...
open_in_excel <- function(some_df) {
  tFile <- paste("C:/temp/", gsub("\\\\", "", tempfile(
    fileext = paste0(substitute(some_df), ".csv"), tmpdir = ""
  )), sep = "")
  write.table(some_df,
              tFile,
              row.names = F,
  ...
  system(
    paste(
      'open -a \"/ProgramData/Microsoft/Windows/Start Menu/Programs/Microsoft Office/Microsoft Excel 2010\"',
      tFile
    )
  )
}
...
colors_EV <- colorRampPalette(c(bleu_EV, turquoise_EV, jaune_EV, orange_EV))
# chargement des données du rapport anguilles (nommé vvv)

load(str_c("C:/workspace/passe_anguilles/data/", CY, "/vvv.Rdata"))
@
%
%
...
\section{Résultats}
<<chargement,eval=TRUE,include=FALSE>>=
...
stationsrepetees <- as.character(pec$Id_PtPrelMetier)[strftime(pec$Op_Dtd, "%Y") ==
                                                        2007 | strftime(pec$Op_Dtd, "%Y") >= 2014]
...
#pec[pec$annee==2021,"Id_PtPrelMetier"][order(pec[pec$annee==2021,"Id_PtPrelMetier"])]

# problème avec le cable pas dans l'eau en première pêche
# 32 première pêche 74 deuxième pêche donne un NCS de 1229
# j'inverse premier et deuxième.
# Eff1 <- pec[pec$Op_code=="20020000476","Eff1"] 74
# Eff2 <- pec[pec$Op_code=="20020000476","Eff2"] 32
#corrigé avec Brice
pec[pec$Op_code=="20020000476","Eff1"] <- 80
pec[pec$Op_code=="20020000476","Eff2"] <- 26
# 50 % d'efficicacité pour des bonnes conditions de pêches et pas mal de petites
# en 1998
...
pec$biomCS <- pec$poidstotal * (pec$NCS / (pec$Eff1 + pec$Eff2 + pec$Eff3calcul)) /
  (pec$Op_Lng * pec$Op_Lam)

pec[, c("age0", "age1", "age2", "age3", "age4")] <- pec[, c("age0", "age1", "age2", "age3", "age4")] *
  (pec$NCS / ((pec$Eff1 + pec$Eff2 + pec$Eff3calcul) * pec$Op_Lng * pec$Op_Lam))
library(ggplot2)
library(lattice)
...
stations = read.table(str_c(datawd, "summary_station_short.csv"),
                      sep = ";",
                      header = TRUE)
...
# Vérification des corrections pour le Trévelo, correspond à l'efficacité moyenne (40 %)
mean(dens[dens$Pm_Id_PointPrel ==  87684 & dens$Op_Dtd != "2024-09-18","eff"])
dens[dens$Pm_Id_PointPrel ==  87684 & dens$Op_Dtd == "2024-09-18","eff"]
dens <- chnames(dens, "code_iav", "station")
# fix error 2023 2024 (inversion des stations 21 et 20 sur WAMA.
dens[dens$annee %in% c(2023, 2024) & dens$station =="SEICH-150","code_histo"] <- "0435XX21"
dens[dens$annee %in% c(2023, 2024) & dens$station =="SEICH-150","station"] <- "SEICH-149"
...
<< tableau1, eval = TRUE, include = FALSE >>=
...
\begin{table}[htpb]
\centering
\caption{Captures de la pêcherie de civelles d'Arzal de 1995 à \Sexpr{CY}, sources : 
...
<<loadseriesfromserver,eval=FALSE, include=FALSE>>=
# NOTE LE TABLEAU N'EST PROBABLEMENT PAS A JOUR. IL FAUDRAIT METTRE EN BASE PROPRE CES ELEEMENTS
{}
...
## décommenter pour recharcher les données
serie <- read_excel(
  path = str_c(
    "T:/12-PoleMAB/Socle/Migrateurs/anguille/pecherie/",
    "serie_pro_vilaine.xlsx"
  ),
  sheet = "bilan_gestion",
  range = str_c("A2:AE", CY - 1996 + 4)
)
serie <- serie[-1, ]  # lignes avec kg ou effectif
serie <- killfactor(serie)
serie[2:ncol(serie)] <- apply(serie[2:ncol(serie)], 2, function(x)
  as.numeric(x))
...
serie$passe_relachees_n <- rowSums(
  cbind(
    serie$passe_total_n,
    -serie$passe_mortes_n,
    -serie$passe_marquage_n
  ),
  na.rm = TRUE
)
serie[, "passe_total_kg"] <- vvv$CIV$data$Pgl
serie$passe_relachees_kg <- rowSums(
  cbind(
    serie$passe_total_kg,
    -serie$passe_mortes_kg,
    -serie$passe_marquage_kg
  ),
  na.rm = TRUE
)
...
serie$saison <- gsub("-", "_", serie$saison)
serie[serie$annee > 2017, c("transport_CLPMEM_kg", "transport_CLPMEM_n")] <- NA # transport Aulne
...
<<figure2recrutement,eval=FALSE, include=FALSE>>=
...
serie$recr_flu_aj_transport = fnrec(
  mortpas = 1,
  mortxp = 0,
  mortecl = 1,
  data = serie,
...
serie$recr_flu_aj_transport_kg = fnrec(
  mortpas = 1,
  mortxp = 0,
  mortecl = 1,
  data = serie,
...
points(serie$recr_flu_aj_m3,
       type = "l",
       col = "deepskyblue")

#-----------------------------------------------------------------
pdf(
  file = str_c(imgwdy, "figure2recrutement.pdf"),
  width = 7,
  height = 6
)
...
xx <- c(x, rev(x))
yy <- c(rep(0, l), rev(y))
polygon(xx, yy, col = orange_EV, border = FALSE)
lines(x, y, lwd = 2.5, col = orange_EVf)
at <- axTicks(2)
at3 <- at / 10^6
at2 <- at / 10^3
at1 <- round(at / 25000)
...
axis(2,
     at,
     labels = formatC(at2, big.mark = " ", format = "d"),
     las = 3)
axis(
  side = 4,
  at = at,
...
fin <- 2011.5
polygon(
  c(debut, debut, fin, fin),
  c(maxo, mino, mino, maxo),
  col = "#FA050535",
  border = FALSE
)
text(fin - (fin - debut) / 2,
     8 * maxo / 10,
     iconv("arrêt \n 31/03", "UTF8"),
     col = "red")
...
mtext(expression("Anguille jaunes x  " * 1000),
      4,
      line = 3,
      col = bleu_EV)
...
symbols(
  x = c(2007, 2012, 2013),
  y = (serie$eclusage_n)[c(12, 17, 18)],
  squares = c(0.3, 0.3, 0.3),
  add = TRUE,
  inches = F,
...
symbols(
  x = x[serie$recr_flu_aj_transport > 0],
  y = (serie$recr_flu_aj_transport)[serie$recr_flu_aj_transport > 0],
  stars =  matrix(
    rep(
      c(0.7, 0.1, 0.7, 0.1, 0.7, 0.1, 0.7, 0.1, 0.7, 0.1) / 2,
...
#-----------------------------------------------------------------
pdf(
  file = str_c(imgwdy, "figure2recrutement_kg.pdf"),
  width = 7,
  height = 6
)
par(mar = c(5.1, 5.1, 4.1, 5.1))
...
polygon(xx, yy, col = orange_EV, border = FALSE)
lines(x, y, lwd = 2.5, col = orange_EVf)
points(x, y, col = orange_EVf, lwd = 1.5, pch = 20)
grid(nx = NA, ny = NULL)
at <- axTicks(2)
at1 <- round(at / 25000)
axis(1, at = x)
axis(2,
     at,
     labels = formatC(at, big.mark = " ", format = "d"),
     las = 3)
...
symbols(
  x = c(2007, 2012, 2013),
  y = (serie$eclusage_kg)[c(12, 17, 18)],
  squares = c(0.3, 0.3, 0.3),
  add = TRUE,
  inches = F,
  bg = c("white", "white", "white")
...
symbols(
  x = x[serie$recr_flu_aj_transport_kg > 0],
  y = (serie$recr_flu_aj_transport_kg)[serie$recr_flu_aj_transport_kg > 0],
  stars =  matrix(
    rep(
      c(0.7, 0.1, 0.7, 0.1, 0.7, 0.1, 0.7, 0.1, 0.7, 0.1) / 2,
...
$11~203$ civelles  (soit $2.6 kg$) en $2011$ à
$4~727~345$ (soit $1.6t$) en $2014$. 
Le nombre d'anguilles jaunes comptées sur les passes a varié de $1~016$
à $145~103$ entre la plus mauvaise année -$2015$- et la
meilleure année -$2013$.
...
d'Arzal. La même figure est reportée en kg
...
(2009-2010). En moyenne, le taux d'exploitation est de 67\% et ce taux
est donc supérieur à la cible de gestion fixée par le règlement européen (40 \% d'échappement soit
...
<<tableau2_dens_eff_annee, eval=TRUE, include=FALSE>>=
#####################################
...
(
  tableau2 <- data.frame(
    "Nb" = tapply(pec$densCS, pec$annee, function(X)
      length(X)),
    ...
    "$IC~D$" = round(
      tapply(pec$densCS, pec$annee, function(X)
        t.test(X)$conf.int[2] - t.test(X)$estimate),
      2
    ),
    ...
    "IC~$\\Phi$" = round(
      tapply(pec$eff, pec$annee, function(X)
        t.test(X)$conf.int[2] - t.test(X)$estimate),
      2
    ),
    "$B_e$" = round(tapply(pec$biomCS, pec$annee, mean, na.rm = TRUE), 2),
    "$IC~B_e$" = round(
      tapply(pec$biomCS, pec$annee, function(X)
        t.test(X)$conf.int[2] - t.test(X)$estimate),
      2
    )
  )
)
colnames(tableau2) <- c("Nb",
                        "annee",
                        "$D$",
                        "$IC~D$",
                        "$\\Phi$",
                        ...
                        "$B_e$",
                        "$IC~B_e$")
write.table(tableau2, str_c(tabwdy, "tableau_densite.csv"), sep = ";")
options(encoding = "UTF-8")
xtableau2 <- xtable(tableau2,
                    label = "tableau2_dens_eff_annee",
                    caption = c(
                      str_c(
                        "Densités ($D$) et biomasses moyennes $B_e$ en anguilles (méthode Carle et Strub) et efficacités de pêche $\\Phi$ calculées pour
								les 19 stations prospectées entre 1998 et ",
								CY,
...
print(
  xtableau2,
  file = str_c(tabwdy, "tableau2_dens_eff_annee.tex"),
  table.placement = "htbp",
  caption.placement = "top",
  NA.string = "",
  include.rownames = FALSE,
...
@
<<analysestat, eval=FALSE, include=FALSE>>=
#############################
...
summary(glmdens <- glm(log(densCS + 0.01) ~ station + annee + mois2, data =
                         dens[dens$densCS > 0, ]))
shapiro.test(resid(glmdens)) # 0.6 OK ouf !!!
crPlot(
  model = glmdens,
  terms =  ~ annee,
  main = "Component residual plot for glm (log(densite) ~ station + annee",
  variable = "annee"
)
...
m.lm <- gls(log(densCS) ~ station + annee + mois2, data = dens[dens$densCS >
                                                                 0, ])
summary(m.lm)
varianceState <- varIdent(form =  ~ 1 | station)
m.gls1 <- gls(log(densCS) ~ station + annee + mois2,
              weights = varianceState,
              data = dens[dens$densCS > 0, ])
...
E <- resid(m.gls1)
coplot(E ~ annee |
         station, data = dens[dens$densCS > 0, ]) # il n'y a plus de problème
m.gls2 <- gls(log(densCS) ~ station + mois2,
              weights = varianceState,
              data = dens[dens$densCS > 0, ])
anova(m.gls2, m.gls1) # pas mieux bien sûr
AIC(m.gls2, m.gls1)
m.gls3 <- gls(log(densCS) ~ annee + riviere,
              weights = varianceState,
              data = dens[dens$densCS > 0, ])
anova(m.gls3, m.gls1) # pas mieux bien sûr
m.gls4 <- gls(log(densCS) ~ station + annee,
              weights = varianceState,
              data = dens[dens$densCS > 0, ])
...
%
%
<< figure7_anx_glht, eval =  TRUE, include = FALSE >>=
  pdf(
    file = str_c(imgwdy, "clddensiteannuelle.pdf"),
    height = 5,
    width = 7
  )
...
#
#par(old.par)
#dev.off()@<  < figure_densite_annee, eval = TRUE, include = FALSE >  >=
########################################
...
coef <- coef[1] + coef[2:length(coef)] # ajout de l'intercept
coef <- c(coef, "annee1998" = 0)
df_coef <- data.frame("annee" = gsub("annee", "", names(coef)),
                      "coefficient" = coef)
dens$resid[dens$densCS > 0] <- resid(m.gls1)
densmod <- merge(dens, df_coef, by = "annee")
rm("df_coef", "coef")
# component + residual
...
densmod <- merge(densmod, df_letter, by = "annee")
rm(df_letter)
colnames(tableau2)[c(3, 4)] <- c("D", "IC_D")
densmod$anneec <- as.POSIXct(strptime(densmod$annee, format = "%Y"))
tableau2$anneec <- as.POSIXct(strptime(tableau2$annee, format = "%Y"))

pdf(
  file = str_c(imgwdy, "dens_annee.pdf"),
  height = 5,
  width = 9
)
g <- ggplot(densmod, aes(x = anneec, y = densCS))
g + geom_point(col = "grey40", size = 1.8) +
  ...
  xlab("Année") +
  guides(fill = guide_legend(title = "Groupe (Tukey)")) +
  theme_bw()
#ggtitle("Tendance des densités en fonction des années")
dev.off()
@
%
<<table_densite_annee_dist,eval=TRUE, include=FALSE>>=

me <- round(tapply(dens$densCS, list(dens$classdist, dens$annee), mean, na.rm =
                     TRUE), 2)
iconf <- round(tapply(dens$densCS, list(dens$classdist, dens$annee), function(X)
  t.test(X)$conf.int[2] - t.test(X)$estimate),
  2)
tableau3 <- t(me)
for (i in 1:nrow(me)) {
...
xtableau3 <- xtable(tableau3,
                    ...
                    caption = c(
                      str_c(
                        "Densités moyennes en anguille.m$^{-2}$ en fonction de la distance (+ - intervalles de confiance à 0.05.)"
                      )
                      ,
                      "Densité et distance."
                    ))
print(
  xtableau3,
  file = str_c(tabwdy, "tableau3_dens_dist_annee.tex"),
  table.placement = "htbp",
  caption.placement = "top",
  NA.string = "",
  include.rownames = FALSE,
...
@
%
<<figure_densite_annee_dist,eval=TRUE, include=FALSE>>=
########################################
#  Figure Tendance des densités en fonction des années et de la distance mer
########################################
mean_densities <- melt(t(me),
                       varnames = c("annee", "classdist"),
                       value.name = "densCS")
mean_densities$anneec <- as.POSIXct(strptime(mean_densities$annee, format =
                                               "%Y"))
mean_densities$anneef <- as.factor(mean_densities$annee)
dens$anneec <- as.POSIXct(strptime(dens$annee, format = "%Y"))
dens$anneef <- as.factor(dens$annee)

pdf(
  file = str_c(imgwdy, "dens_annee_dist.pdf"),
  height = 5,
  width = 7
)
require(viridis)
g <- ggplot(dens, aes(x = anneec, y = densCS))
g + geom_boxplot(aes(fill = anneef)) +
  geom_point(
    aes(x = anneec, y = densCS),
    data = mean_densities,
    col = "red",
    shape = 3
  ) +
...
@
%
<<figurebiom_annee, eval=TRUE, include=FALSE>>=
########################################
...
pdf(
  file = str_c(imgwdy, "biom_annee.pdf"),
  height = 7,
  width = 9
)
g <- ggplot(dens, aes(x = anneec, y = biomCS))
g + geom_boxplot(aes(group = anneec, fill = anneef)) +
  scale_fill_viridis(name = "group",
                     discrete = TRUE,
                     alpha = 0.7) +
...
@
%
<< figure_biom_annee_dist, eval = TRUE, include = FALSE >>=
  ########################################
#  Figure  Tendance des biomasses en fonction des années et de la distance mer
########################################
#cols<-rep(RColorBrewer::brewer.pal(12,"Set3"),4)[1:length(unique(dens$annee))]
pdf(
  file = str_c(imgwdy, "biom_annee_dist.pdf"),
  height = 5,
  width = 9
)
g <- ggplot(dens, aes(x = anneec, y = biomCS))
g + geom_boxplot(aes(fill = anneef, group = anneec)) +
  facet_grid( ~ classdist) +
  scale_fill_viridis(name = "group",
                     discrete = TRUE,
                     alpha = 0.7) +
...
dev.off()
@
<<table_biom_annee_dist,eval=TRUE, include=FALSE>>=
# biomasseCS et IC Ã  0.05
me <- round(tapply(dens$biomCS, list(dens$classdist, dens$annee), mean, na.rm =
                     TRUE), 2)
iconf <- round(tapply(dens$biomCS, list(dens$classdist, dens$annee), function(X)
  t.test(X)$conf.int[2] - t.test(X)$estimate),
  2)
tableau4 <- t(me)
for (i in 1:nrow(me)) {
...
xtableau4 <- xtable(tableau4,
                    ...
                    caption = c(
                      str_c(
                        "Biomasses moyennes en anguilles (en g.m$^{-2}$) en fonction de la distance (+ - intervalles de confiance à 0.05)."
                      )
                      ,
...
print(
  xtableau4,
  file = str_c(tabwdy, "table_biom_dist_annee.tex"),
  table.placement = "htbp",
  caption.placement = "top",
  NA.string = "",
  include.rownames = FALSE,
...
@
%
<<figure_densite_age_annee,eval=TRUE, include=FALSE>>=
########################################
...
data_age <- melt(
  ...
  id.vars = c("Op_code", "annee", "anneec", "anneef", "classdist"),
  variable_name = "age"
)
colnames(data_age) <- c("Op_code",
                        "annee",
                        "anneec",
                        "anneef",
                        "classdist",
...
pdf(
  file = str_c(imgwdy, "dens_age_annee.pdf"),
  height = 5,
  width = 7
)
...
dev.off()
@
<<table_densite_age_annee,eval=TRUE, include=FALSE>>=
data_age <- melt(
  dens[, c(
    "Op_code",
    "annee",
...
me <- round(tapply(
  data_age$densiteCS,
  list(data_age$age, data_age$annee),
  mean,
  na.rm = TRUE
), 2)
iconf <- round(tapply(data_age$densiteCS, list(data_age$age, data_age$annee), function(X)
  t.test(X)$conf.int[2] - t.test(X)$estimate),
  2)
tableau5 <- t(me)
for (i in 1:nrow(me)) {
...
xtableau5 <- xtable(tableau5,
                    ...
                    caption = c(
                      str_c(
                        "Densité moyennes en anguilles (en anguille.m$^{-2}$) en fonction de la distance à la mer et de l'âge + - intervalles de confiance à 0.05."
                      )
                      ,
...
print(
  xtableau5,
  file = str_c(tabwdy, "table_densite_age_annee.tex"),
  table.placement = "htbp",
  caption.placement = "top",
  NA.string = "",
  include.rownames = FALSE,
...
options(encoding = "native.enc")
@
<<figure_age_annee_dist,eval=TRUE, include=FALSE>>=
########################################
# Figure  Tendance des densités en fonction de l'âge et de la distance mer
########################################
pdf(
  file = str_c(imgwdy, "dens_age_annee_dist.pdf"),
  height = 8,
  width = 8
)
g <- ggplot(data_age, aes(x = age, y = densiteCS))
...
dev.off()
@
<<figure_anx_densite_annee_riviere,eval=TRUE, include=FALSE>>=
########################################
# Figure (annexe ?) Tendance des densités en fonction des rivières
########################################
#cols<-rep(RColorBrewer::brewer.pal(12,"Set3"),4)[1:length(unique(dens$annee))]
pdf(
  file = str_c(imgwdy, "dens_annee_riviere.pdf"),
  height = 6,
  width = 8
)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\small
\input{../pechelec/table/2024/tableau2_dens_eff_annee.tex}
\normalsize
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
Entre 2015 et 2024, elle varie entre \Sexpr{tableau2["2017",3]} en
2017 et \Sexpr{tableau2["2024",3]} en 2024.
...
l'estuaire avec en 2021 \Sexpr{tableau3["2021",2]} anguille.m$^{-2}$. Les
densités moyennes dépassent également après 2013 le seuil de 0.3
anguille.m$^{-2}$ pour la zone intérmédiaire (2021 50-100 rkm \Sexpr{tableau3["2021",3]}
...
\Sexpr{tableau3["2021",4]} anguille.m$^{-2}$) (Tableau \ref{tableau3_dens_dist_annee}). 
...
\Sexpr{tableau2["2021",7]}  (+-\Sexpr{tableau2["2021",8]}) g.m$^{-2}$.
...
\small
\input{../pechelec/table/2024/tableau3_dens_dist_annee.tex}
\normalsize
...
\small
\input{../pechelec/table/2024/table_biom_dist_annee.tex}
\normalsize
...
les pêches électriques de 1998 à \Sexpr{CY} \footnotemark}.
...
<<marquage,eval=TRUE, include=FALSE>>=
...
summary_station <- read.csv(
  file = str_c(datawd, "summary_station.csv"),
  sep = ";",
  header = T,
  stringsAsFactors = FALSE
)
summary_station <- chnames(summary_station, "code_histo", "station")
marq <- killfactor(read.table(
  file = str_c(datawdy, "marq.csv"),
  stringsAsFactors = FALSE,
  header = TRUE,
  sep = ";"
))
...
nrow(marq) # 740 #800 (2023) 838 (2024)
stopifnot(nrow(marq$station[!is.element(marq$station, summary_station$station)])==0)
...
sum(marq$marque %in% marqr) #92 (2015) 126 (2016) 206(2017) 244(2018) 282(2020) ? (2021) 343 (2024)
...
options(encoding = "UTF-8")
xta1 <- xtable(ta1, label = "tablemr", caption = c(
  str_c("Marquage et recaptures."),
  str_c(
    "Marquage et recaptures. Les marques sont soit posées, soit lues lorsque l'anguille est déjà marquée."
  )
))
digits(xta1) <- c(0, 0, 0)
align(xta1) <- c("l", "r", "r")
...
options(encoding = "native.enc")
counts <- marq2 %>% group_by(marque) %>%	summarize(nrecapt = n()) %>% mutate(nrecapt =
                                                                               nrecapt - 1)
marq3 <- plyr::join(marq2, counts)
png(
  file = str_c(imgwdy, "croissance.png"),
  height = 6,
  width = 6,
  units = "in",
  res = 300
)
ggplot(marq3) + geom_point(aes(y = taille, x = date, col = riviere)) +
  geom_path(aes(
    y = taille,
    x = date,
    group = marque,
  ...
  scale_colour_manual(values = colors_EV(length(unique(marq3$riviere)))) +
  theme_bw() +
  ylab("taille (mm)") +
  facet_grid( ~ nrecapt)
dev.off()
...
croissance <- plyr::join(
  ...
  mutate(
    croissance_annuelle_mm = croissance_taille / duree_an,
...
croissance$croissance_annuelle_mm[croissance$croissance_annuelle_mm < 0] <-
  0
croissance$croissance_taille[croissance$croissance_taille < 0] <- 0
png(
  file = str_c(imgwdy, "croissance2.png"),
  height = 6,
  width = 6,
  units = "in",
  res = 300
)
ggplot(croissance) + geom_point(aes(x = taille0, y = croissance_annuelle_mm),
                                col = "black",
                                size = 2.5) +
  geom_point(aes(x = taille0, y = croissance_annuelle_mm, col = classdist),
             size = 2.5) +
  ylab("croissance en mm") + xlab("taille au moment du marquage (mm)") +
  scale_color_manual("station", values = c(orange_EV, bleu_EV, turquoise_EV)) +
...
png(
  file = str_c(imgwdy, "croissance3.png"),
  height = 6,
  width = 6,
  units = "in",
  res = 300
)
ggplot(croissance) + geom_boxplot(aes(x = annees, y = croissance_annuelle_mm, group =
                                        annees), fill = turquoise_EV) +
  ylab("croissance en mm") + xlab("duree du marquage") +
  
  theme_bw()
dev.off()
t_test_croissance <- t.test(
    x = croissance$croissance_annuelle_mm[croissance$annees == 2],
    y = croissance$croissance_annuelle_mm[croissance$annees == 4])
lattice::densityplot(croissance$croissance_annuelle_mm)
croissance$annees <- as.numeric(as.character(croissance$annees))
g0 <- glm(croissance_annuelle_mm ~ classdist + taille0 + annees, data =
            croissance)
g1 <- glm(croissance_annuelle_mm ~ station + taille0 + annees, data = croissance)
g2 <- glm(croissance_annuelle_mm ~ distance_mer + taille0 + annees, data =
            croissance)
...
print(
  ...
  file = str_c(tabwdy, "anova_croissance.tex"),
  sep = "\n"
)
options(encoding = "native.enc")
Anova(g2, type = "III")
...
png(
  file = str_c(imgwdy, "croissance4.png"),
  height = 6,
  width = 6,
  units = "in",
  res = 300
...
crois_p_pos <- mean(croissance$croissance_annuelle_mm[croissance$croissance_annuelle_g >
                                                        0])
TempSUP13 <- 79# (948/12)
Ratiodistsea <- 0.75
...
txrecapt <- round(100 * sum(ta1[2:nrow(ta1), 1]) / sum(ta1[1:(nrow(ta1) -
                                                                1), 2]))
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{../pechelec/table/2023/tableaumr1.tex}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
\input{../pechelec/table/2024/anova_croissance.tex}
...
<<brouillon,eval=FALSE, include=FALSE>>=
...
@
%
<< survies,eval = FALSE,include = FALSE >>=
...
ges$recr_flu_aj_m1 = (ges$passem * (1 - mortpas) + ges$pecxp * (1 - mortxp) +
                        ges$eclm * (1 - mortpas)) * 1e6 + ges$aj * 1000

mortxp = (1 + mortpas) / 2

# In the following we assume the same mortality rate of glass eel from the trapping ladder and of those coming form the sluice
ges$recr_flu_aj_m2 = (ges$passem * (1 - mortpas) + ges$pecxp * (1 - mortxp) +
                        ges$eclm * (1 - mortpas)) * 1e6 + ges$aj * 1000

mortxp = 1

# In the following we assume the same mortality rate of glass eel from the trapping ladder and of those coming form the sluice
ges$recr_flu_aj_m3 = (ges$passem * (1 - mortpas) + ges$pecxp * (1 - mortxp) +
                        ges$eclm * (1 - mortpas)) * 1e6 + ges$aj * 1000

mortxp = 1
mortpas = 1
ges$recr_flu_aj_m4 = (ges$passem * (1 - mortpas) + ges$pecxp * (1 - mortxp) +
                        ges$eclm * (1 - mortpas)) * 1e6 + ges$aj * 1000
library(ggplot2)
sumplussd = function(df, var)
  mean(df$"densCS", na.rm = TRUE) + sd(df$"densCS", na.rm = TRUE)
summinussd = function(df)
  mean(df$"densCS", na.rm = TRUE) - sd(df$"densCS", na.rm = TRUE)
meanspl = function(df)
  mean(df$"densCS", na.rm = TRUE)
meanspl1 = function(df)
  mean(df[, c("1")], na.rm = TRUE)
sumplussd1 = function(df, var)
  mean(df$"1", na.rm = TRUE) + sd(df$"1", na.rm = TRUE)
summinussd1 = function(df)
  mean(df$"1", na.rm = TRUE) - sd(df$"1", na.rm = TRUE)
spl = ddply(dens,
            .variables = c("annee", "classdist"),
            .fun = sumplussd)
smi = ddply(dens,
            .variables = c("annee", "classdist"),
            .fun = summinussd)
...
spl$denscs = ddply(dens,
                   .variables = c("annee", "classdist"),
                   .fun = meanspl)[, 3]
spl$denscs1 = ddply(dens,
                    .variables = c("annee", "classdist"),
                    .fun = meanspl1)[, 3]
spl$summinussd1 = ddply(dens,
                        .variables = c("annee", "classdist"),
                        .fun = summinussd1)[, 3]
spl$sumplussd1 = ddply(dens,
                       .variables = c("annee", "classdist"),
                       .fun = sumplussd1)[, 3]
...
plot(
  ...
  xlab = "annee",
  xaxt = "n",
  ylab = expression("Civelles x " * 1000),
  type = "b",
  yaxt = "n",
  frame = FALSE,
  col = "grey60",
...
symbols(
  x = (1996:2011)[ges$pecxp > 0],
  y = (ges$pecxp * 10^6)[ges$pecxp > 0],
  stars = matrix(rep(
    c(0.5, 0.1, 0.5, 0.1, 0.5, 0.1, 0.5, 0.1, 0.5, 0.1) / 2, 7
  ), 7, 10),
  add = TRUE,
  inches = F,
  bg = "grey"
...
text(
  x,
  y,
  apply(format(round(z3, digits = 2)), 1, paste, collapse = ","),
  adj = c(-.2, 0),
  cex = .75,
  col = "purple",
  xpd = NA
...
barplot(
  ges$recr_flu_aj_m3 / 1e6,
  names.arg = 1996:2011,
  col = "blue",
  ylab = "recrutement fluvial net (millions), densite anguille.m-2"
)
rect(
  xleft = c(3.8, 5, 7.4, 8.6, 9.8, 11.01, 18.2),
  ybottom = (ges$recr_flu_aj_m3 / 1e6)[c(4, 5, 7, 8, 9, 10, 16)],
  xright = c(4.3, 5.5, 7.9, 9.1, 10.3, 11.5, 18.7),
  ytop = (ges$recr_flu_aj_m2 / 1e6)[c(4, 5, 7, 8, 9, 10, 16)],
  col = "slateblue1",
  lty = 2
)
rect(
  xleft = c(4.3, 5.5, 7.9, 9.1, 10.3, 11.5, 18.7),
  ybottom = (ges$recr_flu_aj_m3 / 1e6)[c(4, 5, 7, 8, 9, 10, 16)],
  xright = c(4.8, 6, 8.4, 9.6, 10.8, 12, 19.2),
  ytop = (ges$recr_flu_aj_m1 / 1e6)[c(4, 5, 7, 8, 9, 10, 16)],
  col = "skyblue",
  lty = 2
)
...
barplot(
  ges$recr_flu_aj_m4 / 1e6,
  names.arg = 1996:2011,
  col = "red",
  add = TRUE
)
points((c(0.5:13.5) + 0.2 * (c(1:14)))[c(1:8, 10, 12, 14)],
       (flu$denscs[3:16])[-c(9, 11, 13)],
       pch = 19,
       col = "green",
       type = "b"
)
...
legend(
  6,
  2,
  legend = iconv("densités en pêche électrique Y+2", "utf8"),
  col = "limegreen",
  pch = 19,
  bty = "n"
)
...
Les densités de 2017, 2019 et 2022 laissent suspecter un effet de l'étiage
sévère sur certaines stations aval, où les recaptures ont été très faibles 
...
Les variations de recrutement fluvial ne se
traduisent pas par un effet notable sur les densités d'anguilles, y compris
quand on regarde la conversion en une structure en âge par des clés taille-âge.
...
\section{Annexes}
\small
\input{../pechelec/table/2024/star.tex}
\normalsize
%\begin{figure}[htbp]
%\centering
...
\end{figure}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\input{../pechelec/table/2024/table_densite_age_annee.tex}
...
différents affluents du bassin versant de la Vilaine.}

...
# lance a partir de main_pechelec.R
# Author: cedric
...
# et manque des donnees poissons d'une operation
# le fichier est a copier depuis C:/WamaVB/File
...
path <- "M:/pechelec/Wama.mdb"
file.exists(path)
channel <- odbcConnectAccess(path, uid = "", pwd = "nsldgmi")
...
t_ops2 <- t_ops[t_ops$ Sr_Id_Resea %in% c(1084, 1412, 771), c(
  "CdStationMesureEauxSurface", "CodeSandreRdd", "CdStationTemp", "LbStationMesureEauxSurface", "Pm_Id_PointPrel", "Id_PtPrelMetier", "Pm_Lieu", "Id_Reseau", "CodeSandreRdd", "Id_Opera", "Op_code",
...
# head(ope_pois)
# tail(ope_pois)
# Correction de problemes par Brice
...
# La 20020000129 est deja OK


## OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
# Script sql utilise pour les corrections (lance autant de fois que de lignes pour les poissons anguilles manquantes)
## OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
...
#
# // changement des references d'une station de l'etang de chevre et de l'Aron
...
fun <- function(X) {
  ...
  v <- h$counts # comptage des elements par classe
  return(v)
}
# tapply applique la fonction a toutes les valeurs unique l'index (ici a chaque operation)
...
# verification qu'on ne perd personne en route
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
# sta_ope2[sta_ope2$Op_code=="20020000129",] # pour verification
# sta_ope2[sta_ope2$Op_code=="20020000049",] # pour verification
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
# pec[pec$Op_code=="20020000049",] # pour verification
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# Calcul des effectifs par passage...
# debug :: #
# datpois <- sta_ope_pois; pas <- 3;datres <- pec
# ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
# note on suppose qu'on a eu deux passages systematiquement et trois passages exceptionnellement
# car on a pas trouve le nombre de passages dans la table operation
...
# calcule le poids total non corrige de Carle et Strubbe et non corrige de la surface
